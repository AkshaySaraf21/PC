  ...
  override function validate(phoneNumber : String, p1 : Object, parameters : Map<Object, Object>) : IFieldValidationResult {
    ...
    && phoneCountry != PhoneCountryCode.TC_UNPARSEABLE){
      ...
      if (!PhoneUtil.isViableNumber(phoneNumberOriginal)){
        result.addError(displaykey.Validator.Phone.NotViable(phoneNumber))
        return result
      }
      ...
      switch (isPossible){
        case GWPhoneValidationResult.INVALID:
          result.addError(displaykey.Validator.Phone.Invalid(phoneNumber, phoneCountry))
          break
        case GWPhoneValidationResult.INVALID_COUNTRY_CODE:
          result.addError(displaykey.Validator.Phone.InvalidCountryCode(phoneNumber, phoneCountry))
          break
        case GWPhoneValidationResult.TOO_LONG:
          result.addError(displaykey.Validator.Phone.TooLong(phoneNumber, phoneCountry))
          break
        case GWPhoneValidationResult.TOO_SHORT:
          result.addError(displaykey.Validator.Phone.TooShort(phoneNumber, phoneCountry))
          break
