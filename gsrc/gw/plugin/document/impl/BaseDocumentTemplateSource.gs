...
class BaseDocumentTemplateSource implements InitializablePlugin, IDocumentTemplateSource {
  ...
  override function getTemplateAsStream( strTemplateId : String, locale : ILocale ) : InputStream  {
    ...
    Logger.DOCUMENT.debug("Retrieving template {}, in locale {}", strTemplateId, locale.Code)
    try {
  ...
  private function extractLanguage(valuesToMatch : Map) : ILocale {
    ...
    if (languageObj typeis LanguageType ){
      ...
      return LocaleUtil.toLanguage( languageObj as LanguageType )
    }else{
      return null
    }
  }
  ...
  private function readDescriptor(file:File, serializer:IDocumentTemplateSerializer) : IDocumentTemplateDescriptor  {
    try {
      ...
      Logger.DOCUMENT.error("Could not load a document template descriptor: ${file}", t)
      return null
    }
  }
  ...
  private function getLocalTemplateDescriptors(locale : ILocale) : List<IDocumentTemplateDescriptor> {
    var descriptorFiles = getDescriptorFiles()

    if (descriptorFiles.Empty) {
      //Null indicates a directory path or IO problem. Don't cache in this case
      Logger.DOCUMENT.error("Could not find Document Template Files in {}. Usually this error indicates that either the configured directory does not exist or there was an I/O problem", _vdir)
   ...
   */
  private function getDescriptorFiles() : List<File> {
    ...
    var files : List<File> = topDir.listFiles(\f -> f.Name.toLowerCase().endsWith(DESCRIPTOR_SUFFIX)) as List<File> ?: {}
    Logger.DOCUMENT.debug("Found {} document template discriptors", files.Count)
    return files
  }
