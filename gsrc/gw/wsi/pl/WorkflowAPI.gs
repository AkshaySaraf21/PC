...
class WorkflowAPI  {
  ...
  function invokeTrigger(workflowID : String, triggerKey : String){
    ...
    if (trigger == null) {
      throw new IllegalArgumentException(displaykey.Java.Workflow.Trigger.Invalid(triggerKey))
    }
    if (!workflow.isTriggerAvailable( triggerKey )) {
      throw new EntityStateException(displaykey.Java.Workflow.Trigger.Unavaliable)
    }
  ...
  function isTriggerAvailable(workflowID : String, triggerKey : String) : boolean{
    ...
    var workflow = getWorkflowByIdOrThrow(workflowID)
    return workflow.isTriggerAvailable( triggerKey )
  }  
