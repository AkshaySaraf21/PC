...
uses gw.plugin.contact.ContactResult
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.ABContactAPI
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.types.complex.AddressBookUIDContainer
...
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.faults.AlreadyExecutedException
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.faults.EntityStateException
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.faults.RequiredFieldException
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.faults.SOAPException
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.faults.SOAPSenderException
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.faults.BadIdentifierException
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.faults.DataConversionException
uses wsi.remote.gw.webservice.ab.ab700.abcontactapi.faults.SOAPSenderException
...
class ABContactSystemPlugin implements ContactSystemPlugin {
  ...
  private var _mapper = ContactIntegrationXMLMapper.getInstance()

  /**
   * Notifies ContactManager about the creation of a a new contact and if successsful, updates
  ...
  override function addContact(contact : Contact, payload : String,  transactionId : String) {
    ...
    try {
      ...
      var abXmlBackedInstance = wsi.remote.gw.webservice.ab.ab700.beanmodel.XmlBackedInstance.parse(payload)
  ...
  override function updateContact(contact : Contact, changes : String, transactionId : String) {
    ...
    try {
      ...
      var abXmlBackedInstance = wsi.remote.gw.webservice.ab.ab700.beanmodel.XmlBackedInstance.parse(changes)
  ...
  override function removeContact(contact : Contact, removeInfo : String, transactionId : String) {
    try {
      ...
      var abXmlBackedInstance = wsi.remote.gw.webservice.ab.ab700.beanmodel.XmlBackedInstance.parse(removeInfo)
  ...
  override function retrieveContact(addressBookUID : String, creator : ContactCreator) : Contact {
    ...
    if (contactXml != null) {
      var contactType = ContactIntegrationXMLMapper.getInstance().getNameMapper().getEntityNameFromXSDType(contactXml.EntityType)
  ...
  override function searchContacts(searchCriteria : ContactSearchCriteria) : ContactResult[] {
    ...
    var abSearchCriteria = new ABContactSearchCriteriaInfo()
    abSearchCriteria.sync(searchCriteria)
    try {
      var searchSpec = new ABContactAPISearchSpecInfo()
  ...
  override function findDuplicates(contact : Contact) : DuplicateContactResultContainer {
    var searchSpec = new ABContactAPISearchSpecInfo()
    searchSpec.ChunkSize = PCConfigParameters.MaxContactSearchResults.Value
    searchSpec.TagMatcher.Tags.Entry = {wsi.remote.gw.webservice.ab.ab700.abcontactapi.enums.ContactTagType.Client}
    ...
    return new DuplicateContactResultContainerImpl(callWebService(\api -> {
      var abXmlBackedInstance = wsi.remote.gw.webservice.ab.ab700.beanmodel.XmlBackedInstance.parse(xml.bytes())
  ...
  override function createAsyncMessage(messageContext : MessageContext, contact : Contact, lateBoundABUID : boolean) {
    var contactXML = ContactIntegrationXMLMapper.getInstance().populateXMLFromContact(contact , lateBoundABUID)
  ...
  private function updateAddressBookUIDs(contact : Contact, abuidContainer : AddressBookUIDContainer) {
    ...
    for (tuple in abuidContainer.AddressBookUIDTuples.Entry) {
      ...
      var localEntityName = ContactIntegrationXMLMapper.getInstance().getNameMapper().getEntityNameFromXSDType(tuple.EntityType)
      ...
      if (bean == null) {
        bean = contact.Bundle.add(PCBeanFinder.loadBeanByPublicID(tuple.External_PublicID, entityType))
      }
  ...
  private function retrieveContactXML(addressBookUID : String) : XmlBackedInstance {
    var abContactXML : wsi.remote.gw.webservice.ab.ab700.beanmodel.XmlBackedInstance
    try {
