...
abstract class BaseLocalDocumentMetadataSource
  ...
  */
  ...
  var _metadataDirFirstTime = true
  protected property get MetadataDir() : File {
    var rtn = DocumentConfigUtil.getMetadataDir().toJavaFile()
    if (_metadataDirFirstTime) {
      PLLoggerCategory.DOCUMENT.info("BaseLocalDocumentMetadataSource MetadataDir ${rtn}")
      _metadataDirFirstTime = false
    }
    return rtn
  }
  
  protected property get MetadataFiles() : File[] {
    var rtn = MetadataDir.listFiles().where(\ file -> file.File)
    if (PLLoggerCategory.DOCUMENT.DebugEnabled) {
      PLLoggerCategory.DOCUMENT.debug("BaseLocalDocumentMetadataSource MetadataFiles ${rtn as List<File>}")
    }
    return rtn
  }
    ...
    try {
      ...
      if (xml != null) {
        try {
          ...
          // ignore
          PLLoggerCategory.DOCUMENT.debug(e.Message, e)
        }
      }
    }
  }
