...
class NameLocaleSettings {
  ...
  private static final var DEFAULT_FIELDS : Set<NameOwnerFieldId> = {
  ...
  static private function init(_locale : GWLocale) {
    if (_locale.NameOwnerFieldIds == null) {
      if (not validPCFModes.contains(_locale.PCFMode)) {
        print("Error: In ${_locale.Code}/localization.xml, \"${_locale.PCFMode}\" is not a valid PCFMode")
      }
      if (not validTextFormatModes.contains(_locale.TextFormatMode)) {
        print("Error: In ${_locale.Code}/localization.xml, \"${_locale.TextFormatMode}\" is not a valid textFormatMode")
      }
      ...
      for (visibleField in _locale.VisibleFields) {
        ...
        if (fieldId == null) {
          print("Error: In ${_locale.Code}/localization.xml, \"${visibleField}\" is not a valid visibleField name")
        }
      }
