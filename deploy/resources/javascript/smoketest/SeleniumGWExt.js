...
function SeleniumGW_clickGw(locator, bReturnPageName) {
  ...
  if (!eventWin.Ext.fly(elem).hasCls('g-accordion-item')) {
...
function SeleniumGW_isGWOperationDone(bReturnPageName) {
  ...
  var result = !selenium.waitForAnchor && !currWin.Test.isWaitingForAsync() &&
          currWin.document.readyState == 'complete' && !currWin.Ext.Ajax.isLoading();
